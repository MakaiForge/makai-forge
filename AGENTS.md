...
- **Path**: `/home/cas/Documentos/Makai-forge/tools/venv`
- **Versão**: Python 3.10.15
- **Usar SEMPRE**: `tools/venv/bin/python3`
- **NUNCA** usar Python do sistema

### Comunicação Electron ↔ Python
- **Protocolo**: JSON-lines sobre stdin/stdout
- **Classe bridge**: `tools/Mods_manager/services/makai-rpc.ts` → `MakaiRPC`
- **Servidor**: `tools/Mods_manager/core/server.py` (1142 linhas, 53+ métodos RPC)
- **Spawn**: `MakaiRPC.spawn()` → `cp.spawn("tools/venv/bin/python3", ["server.py", "--stdio"])`
...
- `data/install-api/ForgePipeline/orchestrator/orchestrator.ts` — `installAndScan()` (335 linhas)
- `data/install-api/ForgePipeline/orchestrator/prefix-copier.ts` — `copyFolderToPrefix()` (SHA256)
- `data/install-api/ForgePipeline/orchestrator/prefix-scanner.ts` — `scanPrefixForExes()`
- `data/install-api/ForgePipeline/orchestrator/snapshot.ts` — `takeSnapshot()`
- `data/install-api/ForgePipeline/orchestrator/change-detector.ts` — `findNewExecutables()`
- `data/install-api/ForgePipeline/events/open-game/handle-portable.ts` — `handlePortableGame()`
- `data/install-api/ForgePipeline/events/open-game/execute-installer.ts` — `executeInstaller()`
- `data/install-api/ForgePipeline/events/open-game/download-installer.ts` — `downloadFromCatalog()` + detecção
...
|---|---|---|---|
| `data/install-api/ForgePipeline/orchestrator/orchestrator.ts` | 335 | `installAndScan()` | `play.py` or new `install.py` |
| `data/install-api/ForgePipeline/orchestrator/prefix-copier.ts` | 155 | `copyFolderToPrefix()` | New RPC method |
| `data/install-api/ForgePipeline/orchestrator/prefix-scanner.ts` | 28 | `scanPrefixForExes()` | New RPC method |
| `data/install-api/ForgePipeline/orchestrator/snapshot.ts` | ~40 | `takeSnapshot()` | New utility |
| `data/install-api/ForgePipeline/orchestrator/change-detector.ts` | ~30 | `findNewExecutables()` | New utility |
| `data/install-api/ForgePipeline/events/open-game/download-installer.ts` | 104 | `downloadFromCatalog()` + detecção | `detect_installer_type()` + keep download in TS |
| `data/install-api/ForgePipeline/events/open-game/handle-portable.ts` | 59 | `handlePortableGame()` | New RPC method |
| `data/install-api/ForgePipeline/events/open-game/execute-installer.ts` | 56 | `executeInstaller()` | New RPC method |

### Arquivos TypeScript que FICAM no Electron

| Arquivo TS | Motivo |
|---|---|
| `data/install-api/AddGame/add-custom-game-to-library.ts` | Store/DB operations (UI domain) |
| `data/install-api/ForgePipeline/events/open-game/open-game.ts` | Orquestração fina (UI flow) |
| `data/install-api/ForgePipeline/events/open-game/ensure-proton.ts` | Download management (UI domain) |
| `data/install-api/ForgePipeline/events/open-game/handle-prefix.ts` | Window management (UI domain) |
...
| `data/install-api/AddGame/add-custom-game-to-library.ts` | Adiciona jogo custom |
| `data/install-api/ForgePipeline/events/open-game/open-game.ts` | Abrir/reparar jogo |
| `data/install-api/ForgePipeline/services/makai-time.ts` | Bridge MakaiTime → MakaiRPC |
| `tools/Mods_manager/services/makai-rpc.ts` | Classe MakaiRPC (bridge Electron↔Python) |
| `src/preload/library.ts` | API exposta ao renderer |

### Python (Background)
| Arquivo | Função |
|---|---|
| `tools/Mods_manager/core/server.py` | ★ Servidor RPC (53+ métodos) |
| `tools/Mods_manager/core/play.py` | play_game() — fluxo de jogar |
| `tools/Mods_manager/core/detection.py` | detect_game() — detecção de jogo |
| `tools/Mods_manager/core/games_registry.py` | Dados de ~30 jogos |
| `tools/Mods_manager/core/engine/launch.py` | launch_game() → Makai Time |
| `tools/Mods_manager/core/engine/proton.py` | find_proton() |
| `tools/Mods_manager/core/engine/makaitricks.py` | run_multiple() |
| `tools/prefix/python/prefix/core.py` | create_prefix(), delete_prefix() |
| `tools/prefix/python/prefix/makaitricks.py` | DLL management |
| `tools/prefix/python/prefix/container.py` | build_bwrap_cmd() |
| `tools/prefix/python/prefix/runner.py` | Proton runner |
| `tools/prefix/makai_time/makai_time.py` | ★ Entry point Makai Time (run()) |
| `tools/prefix/makai_time/core/gpu.py` | GPU detection |
| `tools/prefix/makai_time/core/sync.py` | ntsync/fsync/esync |
| `tools/prefix/makai_time/core/runtime.py` | Steam Runtime download |
| `tools/prefix/makai_time/overrides/capture.py` | GPU overrides |
| `tools/prefix/makai_time/proton/intel.py` | Proton Intelligence (fork detection) |
| `tools/prefix/makai_time/proton/definitions/*.py` | 31 definições de Proton |
...
# Testar RPC server
echo '{"id":1,"method":"ping"}' | tools/venv/bin/python3 \
  tools/Mods_manager/core/server.py

# Ver venv
tools/venv/bin/python3 --version  # Deve ser 3.10.15
```

## RED FLAGS (NUNCA IGNORAR)
- **NÃO criar Proton próprio** — usar catálogo (20+ forks em tools.ts)
- **NÃO usar Python do sistema** — sempre `tools/venv/bin/python3`
