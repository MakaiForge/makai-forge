...
- `app/_main/container/makai_time/overrides/detect.py` — nova função `egl_vendor_jsons()`
- `app/_main/container/makai_time/overrides/capture.py` — nova função `capture_glvnd_egls()`, chamada em `capture_all_graphics()`
- `app/_main/container/makai_time/overrides/mount.py` — bind de GLVND JSONs em `override_bwrap_args()`

### Verificação
```bash
# Rodar makai_time com --dry-run para NTE e verificar se GLVND JSONs aparecem nos args
cd /home/cas/Documentos/Makai-forge
source app/_venv/bin/activate
...
### O que fazer
- Criar módulo `app/_main/container/makai_time/overrides/resolve.py`
