...
| Arquivo | Descrição |
|---|---|
| `app/_main/container/makai_time/makai_time.py` | Entry point: build_bwrap_cmd() + run() |
| `app/_main/container/makai_time/core/gpu.py` | Detecção de GPU (info apenas) |
| `app/_main/container/makai_time/core/ldso.py` | LD_LIBRARY_PATH + ld.so.cache |
| `app/_main/container/makai_time/core/display.py` | Mount de X11, Wayland, PipeWire, PulseAudio |
| `app/_main/container/makai_time/core/layers.py` | Vulkan/OpenXR layer masking |
| `app/_main/container/makai_time/core/runtime.py` | Resolução de runtime (scout→soldier→sniper→steamrt4) |
| `app/_main/container/makai_time/overrides/detect.py` | Detecção de libs GPU no host |
| `app/_main/container/makai_time/overrides/capture.py` | Captura de libs GPU via symlinks |
| `app/_main/container/makai_time/overrides/mount.py` | Geração de args bwrap para overrides + devices |
| `app/_main/container/makai_time/overrides/capsule_capture.py` | Captura alternativa (cópia recursiva com resolução ELF) |
| `app/_main/container/makai_time/proton/config.py` | Env vars do Proton (NVIDIA, AMD, Intel, sync) |
| `app/_main/container/makai_time/proton/intel.py` | Proton intelligence (32 forks) |
| `app/_main/container/makai_time/proton/anticheat/games/neverness_to_everness.py` | Definição NTE (ACE, DW-Proton, container_relax) |
