...
**Origem dos dados**: `app/_data/games-data/` (~12.200 JSONs) + `data/catalogs/` (análise de forks)
...
### 2.3 Games (`app/_main/installer-api/Games/` — A ABA "JOGOS")
| Arquivo | Função |
|---|---|
...
**Service**: `app/_main/installer-api/AddGame/games-service.ts` (232 linhas)
...
src/main/events/games/index.ts → openGame()
  │
  ▼
app/_main/installer-api/ForgePipeline/events/open-game/open-game.ts (128 linhas)
  │
...
|---|---|---|
| `app/_main/installer-api/ForgePipeline/events/open-game/open-game.ts` | 128 | **ORQUESTRADOR**: decide fluxo, chama Python |
| `app/_main/installer-api/ForgePipeline/events/open-game/ensure-proton.ts` | 56 | Baixa/verifica Proton |
| `app/_main/installer-api/ForgePipeline/events/open-game/download-installer.ts` | 104 | Download do catálogo + detecção installer/portátil |
| `app/_main/installer-api/ForgePipeline/events/open-game/handle-prefix.ts` | 91 | Create prefix, scan prefix, show executable select |
| `app/_main/installer-api/ForgePipeline/events/open-game/send-progress.ts` | — | Envia progresso para launcher window |
| `app/_main/installer-api/ForgePipeline/services/makai-time.ts` | 139 | **BRIDGE**: MakaiTime.runExecutable(), .installGame(), .runInstaller() |
| `app/_main/installer-api/ForgePipeline/services/download/` | — | Sistema de download completo |
| `app/_main/installer-api/ForgePipeline/orchestrator/prefix-setup.ts` | — | setupPrefix() |
| `app/_main/installer-api/AddGame/add-custom-game-to-library.ts` | 166 | Adiciona jogo custom ao store SQLite |
| `app/_main/installer-api/AddGame/games-service.ts` | 232 | CRUD de jogos + GameConfig (77 campos) |

### 3.3 Arquivos Envolvidos (Python — a lógica real)

| Arquivo | Métodos RPC | Linhas | Função |
|---|---|---|---|
| **`app/Catalogo/GameMod/core/game_install.py`** | — (chamado por server.py) | 628 | **CORPO DA INSTALAÇÃO**: 6 funções |
| `app/Catalogo/GameMod/core/server.py` | `detect_installer_type`, `copy_to_prefix`, `scan_prefix_for_exes`, `snapshot_prefix`, `find_new_executables`, `install_game` | 1219 | Registro RPC + delega para game_install.py |
| `app/Catalogo/GameMod/core/play.py` | `play_game` (via server.py) | 347 | Play flow: prefix, DLL overrides, Bethesda, Makaitricks, Script Extender, Deploy, Launch |
...
### 4.2 Python Side (`app/Catalogo/GameMod/core/play.py`, 347 linhas)
...
- **Fonte**: `app/_data/games-data/` (~12.200 JSONs)
...
### 5.3 Games (`app/_main/installer-api/Games/`)
...
│                                                          │
│  Electron: app/_main/rpc/makai-rpc.ts      │
│    → spawn("app/_venv/bin/python3", ["server.py",       │
│       "--stdio"])                                        │
│    → JSON request via stdin                              │
│    ← JSON response/events via stdout                     │
│                                                          │
│  Python: app/Catalogo/GameMod/core/server.py               │
│    → Lê JSON da stdin                                    │
...
- `app/_main/installer-api/ForgePipeline/orchestrator/` (se existir)
- `app/_main/installer-api/ForgePipeline/events/open-game/download-installer.ts` (tem lógica de detecção duplicada)
...
1. **Remover código duplicado do TypeScript**: Os arquivos em `app/_main/installer-api/ForgePipeline/orchestrator/` e `events/open-game/` que foram migrados para Python devem ser apagados ou marcados como `@deprecated`.

2. **Centralizar tipos de jogo**: O `GameConfig` (77 campos em `games-service.ts`) e o `LibraryGame` (`@types`) deveriam ser unificados.

3. **`game_install.py` está no lugar certo** mas dentro do Mods_manager. Deveria estar em `app/_main/container/makai_time/engine/python/prefix/` ou em `core/` nível superior. Sugestão: mover para `app/Catalogo/GameMod/core/` (onde já está) — está OK.

4. **Consolidar AddGame**: O fluxo "Adicionar Jogo Custom" está espalhado entre:
   - `app/_main/installer-api/AddGame/add-custom-game-to-library.ts` (criação no store)
   - `app/_main/installer-api/ForgePipeline/events/open-game/open-game.ts` (pós-criação)
   - `app/_main/installer-api/Games/` (UI)
   Sugestão: unificar em `app/_main/installer-api/Games/`
...
├── _main/
│   └── container/                ← Runtime container (migrado de tools/prefix/)
└── ...
```
...
|---|---|---|
| 🔴 | `app/Catalogo/GameMod/core/server.py` | **ÚNICO** servidor RPC (53+ métodos) |
| 🔴 | `app/Catalogo/GameMod/core/game_install.py` | **Lógica de instalação** (628 linhas) |
| 🔴 | `app/Catalogo/GameMod/core/play.py` | **Play flow completo** (347 linhas) |
| 🔴 | `app/_main/container/makai_time/makai_time.py` | **Entry point Makai Time** (719 linhas) |
| 🟡 | `app/_main/installer-api/ForgePipeline/events/open-game/open-game.ts` | Orquestrador Electron (128 linhas) |
| 🟡 | `app/_main/installer-api/ForgePipeline/services/makai-time.ts` | Bridge MakaiTime→RPC (139 linhas) |
| 🟡 | `app/_main/rpc/makai-rpc.ts` | Classe RPC bridge Electron↔Python (245 linhas) |
| 🟢 | `app/_main/installer-api/AddGame/add-custom-game-to-library.ts` | Add game to store (166 linhas) |
| 🟢 | `app/_main/installer-api/Games/` | UI da aba Games |
...
| `@types` | `src/types/` |
| `@provision` | `app/_main/installer-api/` |
| `@mods-manager` | `app/Catalogo/GameMod/` |
...
### 🥇 MAIOR BAGUNÇA: `app/_main/installer-api/` vs `app/Catalogo/GameMod/`

O `app/_main/installer-api/` contém:
...
O `app/Catalogo/GameMod/core/` contém:
...
1. TypeScript em `app/_main/installer-api/ForgePipeline/` (ANTIGA)
2. Python em `app/Catalogo/GameMod/core/game_install.py` (NOVA)

### 🥈 SEGUNDA BAGUNÇA: GameConfig duplicado

O tipo `GameConfig` (77 campos) está em `app/_main/installer-api/AddGame/games-service.ts`, mas `LibraryGame` nos tipos em `src/types/` tem campos similares mas diferentes. Precisam ser unificados.

### 🥉 TERCEIRA BAGUNÇA: Aba Games dividida

A UI da aba Games está em `app/_main/installer-api/Games/` mas os hooks estão em `src/renderer/src/pages/games/` — duas localizações diferentes para a mesma feature.
...
3. **UNIFICAR** `app/_main/installer-api/Games/` com `src/renderer/src/pages/games/`
