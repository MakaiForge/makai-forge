...
O RPC server vive em `app/Catalogo/GameMod/core/server.py` (ÚNICO servidor,
...
|---|---|
| RPC Server | `app/Catalogo/GameMod/core/server.py` |
| Play flow | `app/Catalogo/GameMod/core/play.py` |
| Engine (launch, proton, registry, makaitricks) | `app/Catalogo/GameMod/core/engine/` |
| Storage | `app/Catalogo/GameMod/core/storage.py` |
| Detection | `app/Catalogo/GameMod/core/detection.py` |
| Deploy | `app/Catalogo/GameMod/core/deploy.py` |
| Makai Time entry point | `app/_main/container/makai_time/makai_time.py` |
| Makai Time core | `app/_main/container/makai_time/core/` (container, gpu, sync, runtime, display, ldso) |
| Makai Time overrides | `app/_main/container/makai_time/overrides/` (capture, detect, mount) |
| Makai Time profiles | `app/_main/container/makai_time/profiles/` |
| Makai Time proton | `app/_main/container/makai_time/proton/` (definitions, intel, recommender) |
| Makaitricks | `app/_main/container/makai_time/engine/python/prefix/makaitricks.py` |
| Electron RPC bridge | `app/_main/rpc/makai-rpc.ts` |
...
#### `play_game(game_id, profile?)` → `app/Catalogo/GameMod/core/play.py:play_game()`

| Electron (O QUE REMOVER) | Linhas | O que faz |
|---|---|---|
| `app/Catalogo/GameMod/play/steps/07-launch.ts` | 208-362 | spawn umu-run, proton direto |
| `app/_main/installer-api/ForgePipeline/services/makai-time.ts` | 106 | spawn `python3 -m makai_time.makai_time` |
| `app/_main/installer-api/ForgePipeline/helpers/launch-game.ts` | 58-330 | spawn wine, nativo, MakaiTime |
| `app/_main/installer-api/ForgePipeline/services/umu.ts` | 221 | spawn umu-run |
| `app/_main/rpc/launch-service.ts` | — | spawn umu-run |
| `app/Catalogo/GameMod/games/_shared/launch.ts` | — | spawn umu/proton |

**Python handler**: `app/Catalogo/GameMod/core/play.py:play_game()` (JÁ EXISTE)
...
#### `kill_game(pid?)` → `app/Catalogo/GameMod/core/server.py` → `core/engine/launch.py:kill_game()`

| Electron | Linhas | O que faz |
|---|---|---|
| `app/Catalogo/GameMod/play/steps/07-launch.ts` | 70-100 | killGameProcess, pkill wineserver |
| `app/Catalogo/GameMod/play/index.ts` | 70-78 | MakaiRPC.call("kill_game") + killGameProcess() |
...
#### `game_status(game_id?)` → `app/Catalogo/GameMod/core/server.py:handle_game_status()`
...
| Electron | Linhas | O que faz |
|---|---|---|
| `app/_main/installer-api/ForgePipeline/orchestrator/prefix-setup.ts` | 68 | spawn umu-run wineboot -u |
| `app/_main/container/makai_time/engine/core/init.ts` | — | checkAndCreateWinePrefix |
| `app/_main/container/events/setup-proton-environment.ts` | — | setup completo |
...
| Electron | Linhas |
|---|---|
| `app/_main/container/makai_time/engine/core/dll-overrides.ts` | — |
...
| Electron | Linhas |
|---|---|
| `app/_main/container/makai_time/engine/core/bethesda-registry.ts` | — |
...
| `app/_main/installer-api/ForgePipeline/events/open-game/ensure-proton.ts` | verifica Proton instalado |
...
| Electron | Linhas | O que faz |
|---|---|---|
| `app/_main/installer-api/ForgePipeline/orchestrator/runner.ts` | 26 | MakaiTime.runInstaller() |
| `app/_main/installer-api/ForgePipeline/events/open-game/execute-installer.ts` | — | executa .exe |
...
| Electron | Linhas |
|---|---|
| `app/_main/installer-api/scripts-install/extractor.ts` | — |
...
| Electron | Linhas |
|---|---|
| `app/Catalogo/GameMod/play/steps/05-frameworks.ts` | — |
| `app/_main/rpc/framework-installer.ts` | — |
| `app/_main/installer-api/ForgePipeline/events/open-game-winetricks.ts` | — |
...
| Electron | Linhas |
|---|---|
| `app/Catalogo/GameMod/play/steps/06-skse.ts` | — |
| `app/_main/rpc/skse-downloader.ts` | — |
...
| `runtime_info` | `app/Catalogo/GameMod/core/server.py` | ✅ |
| `health_check` | `app/Catalogo/GameMod/core/server.py` | ✅ |
...
|---|---|---|
| `app/Catalogo/GameMod/play/steps/07-launch.ts` | REMOVER | RPC `play_game` (backend decide método) |
| `app/Catalogo/GameMod/play/index.ts` | MANTER | IPC → RPC `play_game` (já faz) |
| `app/Catalogo/GameMod/play/steps/04-configs.ts` | REMOVER parcial | RPC `create_prefix`, `install_game_dlls` |
| `app/Catalogo/GameMod/play/steps/05-frameworks.ts` | REMOVER | RPC `install_game_dlls` |
| `app/Catalogo/GameMod/play/steps/06-skse.ts` | REMOVER | RPC já existe em `play.py:_step_script_extender` |
| `app/_main/rpc/launch-service.ts` | REMOVER | RPC `play_game` |
| `app/Catalogo/GameMod/games/_shared/launch.ts` | REMOVER | RPC `play_game` |
| `app/_main/rpc/framework-installer.ts` | REMOVER | RPC `install_game_dlls` |
| `app/_main/rpc/skse-downloader.ts` | REMOVER | RPC já existe em Python |
| `app/_main/rpc/external-tool-installer.ts` | REMOVER | RPC `download_file` |
| `app/_main/installer-api/ForgePipeline/services/makai-time.ts` | REMOVER | RPC `play_game` |
| `app/_main/installer-api/ForgePipeline/services/umu.ts` | REMOVER | Makai Time substitui |
| `app/_main/installer-api/ForgePipeline/helpers/launch-game.ts` | REMOVER | RPC `play_game` |
| `app/_main/installer-api/ForgePipeline/orchestrator/runner.ts` | REMOVER | RPC `get_launch_command` |
| `app/_main/installer-api/ForgePipeline/orchestrator/prefix-setup.ts` | REMOVER | RPC `create_prefix` |
| `app/_main/installer-api/ForgePipeline/events/open-game-winetricks.ts` | REMOVER | RPC `install_makaitricks` |
| `app/_main/installer-api/ForgePipeline/events/open-game/execute-installer.ts` | REMOVER | RPC `get_launch_command` |
| `app/_main/container/makai_time/engine/core/init.ts` | REMOVER | RPC `create_prefix` |
| `app/_main/container/makai_time/engine/core/dll-overrides.ts` | REMOVER | RPC já existe em `play.py` |
| `app/_main/container/makai_time/engine/core/bethesda-registry.ts` | REMOVER | RPC já existe em `play.py` |
| `app/_main/rpc/health-check/index.ts` | REMOVER | RPC `health_check` |
| `app/_main/torrent-rpc/protonforge-api/server.py` | REMOVIDO (stub) | Unificado em server.py |
| `app/_main/installer-api/proton_recommended/python/server.py` | REMOVIDO (stub) | Unificado em server.py |
...
Electron IPC "modPlayGame"                     ← UI layer
         ↓
app/Catalogo/GameMod/play/index.ts               ← IPC handler
  └─ MakaiRPC.call("play_game", {game_id, profile})
         ↓ (JSON-lines stdin)
Python RPC Server (app/Catalogo/GameMod/core/server.py)  ← Backend
  └─ dispatch("play_game")
       └─ core.play.play_game()
