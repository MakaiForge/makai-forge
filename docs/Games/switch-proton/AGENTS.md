...
Em `app/_main/container/makai_time/engine/python/prefix/core.py`:
...
### Python API (app/_main/torrent-rpc/protonforge-api/)
...
### Python API NOVA (app/_main/installer-api/proton_recommended/python/)
- `api/handler.py` — 17 métodos (também precisa dos novos métodos)
- `server.py` — mesmos padrões

### Mod Manager IPC
- `app/Catalogo/GameMod/events/mod-prefix-rpc.ts` — modCreatePrefix, modInstallGameDlls
- `app/Catalogo/GameMod/events/mod-deploy.ts` — deployMods
- `app/Catalogo/GameMod/events/mod-switch-proton.ts` — **NOVO** (não existe ainda)

### Mod Manager Play Pipeline
- `app/Catalogo/GameMod/play/play-game.ts` — orquestrador 8-step
- `app/Catalogo/GameMod/play/steps/03-prefix.ts` — cria prefixo via Python
- `app/Catalogo/GameMod/play/steps/04-configs.ts` — DLL overrides + winetricks
- `app/Catalogo/GameMod/play/steps/06-launch.ts` — spawn proton/umu

### UI
- `app/Catalogo/GameMod/ui/components/GameConfigPanel/GameConfigPanel.tsx` — config do jogo (paths, prefix, health)
- `app/Catalogo/GameMod/ui/components/ProtonConfigPanel/ProtonConfigPanel.tsx` — config simples de Proton
- `app/_main/installer-api/proton_recommended/ui/proton-recommendation-modal.tsx` — seletor de Proton completo (705 linhas)
- `app/Catalogo/GameMod/ui/hooks/config/useProtonConfig.ts` — hook de estado do Proton
