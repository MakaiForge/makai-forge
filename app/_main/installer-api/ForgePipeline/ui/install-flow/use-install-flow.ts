...
export function useInstallFlow() {
  ...
  useEffect(() => {
    const unsubInstall = window.electron.onInstallProgress((value) => {
      setInstallProgress(value);
    });
    const unsubProton = window.electron.onProtonDownloadProgress((value) => {
      if (value) {
        setInstallProgress({
          status: "download",
          percent: value.percent,
          gameTitle: `${value.version} (${value.speed})`,
        });
      }
    });
    return () => {
  ...
  return {
    ...
    showCopyingModal, setShowCopyingModal,
    showInstallSuccessModal, setShowInstallSuccessModal,
    installProgress, setInstallProgress,
    installedProtons, candidates, prefixDriveCPath,
    pendingGameRef, pendingInstallRef, pendingGameIdRef,
    pendingGameTitleRef, suggestedDirRef,
    handleOpenGameInstaller, handleSelectProton,
    handleDownloadAndSelect, handleExePicked,
    handleOpenExePicker, handleNavigateToGames,
  };
}
