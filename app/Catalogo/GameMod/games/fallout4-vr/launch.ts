...
import { FO4_VR_CONSTANTS } from "./fallout4-vr.constants";

export function getGameLaunchEnv(gamePath: string, prefixPath: string, protonPath?: string): Record<string, string> {
  return getSteamLaunchEnv(FO4_VR_CONSTANTS.steamAppId, gamePath, prefixPath, protonPath);
}

export function launchGame(): void {
  launchViaSteam(FO4_VR_CONSTANTS.steamAppId);
}
