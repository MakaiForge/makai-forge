...
export default function Notifications() {
  ...
  const fetchBadges = useCallback(async () => {
    try {
      ...
      const badgesResponse = await window.electron.forgerApi.get<Badge[]>(
  ...
  const fetchApiNotifications = useCallback(
    async (
      ...
      try {
        setIsLoading(true);
        const response =
          await window.electron.forgerApi.get<NotificationsResponse>(
  ...
  const handleMarkAsRead = useCallback(
    async (id: string, source: "api" | "local") => {
      try {
        if (source === "api") {
          await window.electron.forgerApi.patch(
