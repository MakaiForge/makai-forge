...
export function useGameActions(game: LibraryGame) {
      ...
      if (location.pathname === path) {
        try {
          window.dispatchEvent(
            new CustomEvent("protonforge:openRepacks", {
  ...
  const handleToggleFavorite = async (isFavorite = Boolean(game.favorite)) => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("protonforge:game-favorite-toggled", {
  ...
  const handleCreateShortcut = async () => {
    try {
      ...
      const locations = ["desktop"] as const;
  ...
  const handleOpenGameOptions = () => {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("protonforge:openGameOptions", {
          detail: { objectId: game.objectId },
        })
      );
    } catch (e) {
  ...
  const handleOpenWinePrefix = async () => {
    try {
      await window.electron.openGameWinePrefix(game.shop, game.objectId);
    } catch (error) {
      showErrorToast("Failed to open wine prefix folder");
      logger.error("Failed to open wine prefix folder", error);
    }
  };
  ...
  const handleCheckDlls = async () => {
    try {
      ...
      const result = await window.electron.checkGameDlls(game.shop, game.objectId);
      window.dispatchEvent(
      ...
      if (result.installed?.length > 0) {
        showSuccessToast(`DLLs instaladas: ${result.installed.join(", ")}`);
      }
      if (result.errors?.length > 0) {
        showErrorToast(`Erro DLLs: ${result.errors.join("; ")}`);
      }
      if (!result.installed?.length && !result.errors?.length) {
        showSuccessToast("Nenhuma DLL necessaria");
      }
    } catch (error) {
      showErrorToast("Falha ao verificar DLLs");
      logger.error("Failed to check DLLs", error);
      window.dispatchEvent(
  ...
  const handleRemoveFromLibrary = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("protonforge:game-removed-from-library", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch (e) {
  ...
  const handleRemoveFiles = async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("protonforge:game-files-removed", {
            detail: { shop: game.shop, objectId: game.objectId },
          })
        );
      } catch (e) {
