...
export function useSearchHistory() {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        const data = (await storeService.get(SEARCH_HISTORY_KEY, null, "json")) as
          | SearchHistoryEntry[]
          | null;
  ...
  const addToHistory = useCallback(
    (query: string, context: "library" | "catalogue") => {
      ...
      setHistory((prev) => {
        ...
        storeService.put(SEARCH_HISTORY_KEY, updated, null, "json");
        return updated;
      });
    },
    []
  ...
  const clearHistory = useCallback(() => {
    setHistory([]);
    storeService.del(SEARCH_HISTORY_KEY, null);
  }, []);
