...
export interface GameOptionsModalProps {
  ...
  onClose: () => void;
  onNavigateHome?: () => void;
  initialCategory?: GameOptionsCategoryId;
}
...
export function GameOptionsModal({
  ...
  const debounceUpdateLaunchOptions = useRef(
    debounce(async (value: string) => {
      ...
      const gameData = (await storeService.get(
        gameKey,
        "games"
      )) as Game | null;
      if (gameData) {
  ...
  const defaultProtonWinePrefixPath = defaultWinePrefixPath
    ? `${defaultWinePrefixPath}/${game.objectId}`
    : null;
  const displayedWinePrefixPath =
    game.winePrefixPath ?? defaultProtonWinePrefixPath;
          ...
          <div className="game-options-modal__panel">
            {selectedCategory === "general" && (
              <GeneralSettingsSection
                ...
                shouldShowCreateStartMenuShortcut={false}
