    ...
    <div className="games__toolbar">
      ...
      <div className="games__toolbar-group">
        ...
        <div className="games__toolbar-view">
          <button className={viewMode === "grid" ? "active" : ""} onClick={() => onViewModeChange("grid")} title="Cards (padrão)">▦</button>
          <button className={viewMode === "compact" ? "active" : ""} onClick={() => onViewModeChange("compact")} title="Compacto">☰</button>
          <button className={viewMode === "large" ? "active" : ""} onClick={() => onViewModeChange("large")} title="Grande (detalhado)">⊞</button>
        </div>

        {hasHiddenGames && (
          <button className={`games__toolbar-hide ${showHiddenGames ? "" : "active"}`} onClick={onToggleHidden}
        ...
        {onToggleModCompatible && (
          <button className={`games__toolbar-filter ${showModCompatible ? "active" : ""}`} onClick={onToggleModCompatible}
            title={showModCompatible ? "Mostrando apenas jogos compatíveis com mods" : "Mostrando todos os jogos"}>
            🎮{showModCompatible ? "" : " *"}
          </button>
        )}

        <button className="games__toolbar-add" onClick={onAddGame}>+ Adicionar</button>
      </div>
    </div>
  );
}
