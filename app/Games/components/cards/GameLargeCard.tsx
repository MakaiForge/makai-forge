...
export function GameLargeCard({
  thumbnail, portraitUrl, title, runner, playTimeMs, installerSize, installedSize,
  isSteam, appId, onPlay, onClick, onContextMenu, isSelected,
}: LargeCardProps) {
  ...
  const year = details?.release_date?.date
    ? (details.release_date.date.split(",")[1]?.trim() || details.release_date.date.split("-")[0] || details.release_date.date)
    : "";
  ...
  return (
    <div
      ...
      onClick={onClick} onDoubleClick={onPlay} onContextMenu={onContextMenu}
    >
      <div className="game-large-card__art">
        {imgSrc ? <img src={imgSrc} alt={title} loading="lazy" /> : (
          <div className="game-large-card__art-placeholder">{title.charAt(0).toUpperCase()}</div>
        )}
      </div>
      <div className="game-large-card__info">
        ...
        <div className="game-large-card__meta-row">
          ...
          {isSteam && platformStr && <span className="game-large-card__platform">{platformStr}</span>}
          {installerSize != null && installerSize > 0 && (
            <span className="game-large-card__size"><FileZipIcon size={14} />{formatBytes(installerSize)}</span>
          )}
