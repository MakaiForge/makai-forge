...
typedef enum
{
  ...
  PV_LAUNCHER_SERVER_FLAGS_FLATPAK = (1 << 4),
...
{
  /* this unref is paired with the ref in pv_launcher_server_export */
  g_autoptr(PvLauncherServer) self = data;
                  ...
                  gpointer user_data)
{
  ...
  if (pid_data->connection != NULL)
    {
      /* The destination for the signal: the unique name of the client,
       * or NULL on peer-to-peer sockets, or NULL to send a broadcast
       * across the session bus. */
      const char *dest;

      /* Workaround for https://github.com/flatpak/xdg-dbus-proxy/issues/25:
       * normally we'd send a unicast signal like flatpak-portal does,
       * but when running as a Flatpak app, unicast signals to a unique name
       * are blocked by xdg-dbus-proxy, so in this case we use a broadcast. */
      if (self->flags & PV_LAUNCHER_SERVER_FLAGS_FLATPAK)
        dest = NULL;
      else
        dest = pid_data->client;
  ...
  if (!g_spawn_async_with_pipes (arg_cwd_path,
                                 ...
                                 &error))
    {
      ...
      if (g_error_matches (error, G_SPAWN_ERROR, G_SPAWN_ERROR_ACCES))
        ...
        code = G_DBUS_ERROR_SPAWN_FAILED;
  ...
  if (fd == STDIN_FILENO)
    {
      ...
      if (old_stdin < 0)
        return glnx_fd_throw_errno_prefix (error,
                                           "Unable to duplicate standard input");

      if (_srt_fd_set_close_on_exec (old_stdin) < 0)
        return glnx_fd_throw_errno_prefix (error,
                                           "Unable to set flags on fd %d",
                                           old_stdin);

      new_stdin = open ("/dev/null", O_RDONLY | O_CLOEXEC);

      if (new_stdin < 0)
        return glnx_fd_throw_errno_prefix (error, "Unable to open /dev/null");
      ...
      char *argv[])
{
  ...
  if (access ("/.flatpak-info", F_OK) == 0)
    server->flags |= PV_LAUNCHER_SERVER_FLAGS_FLATPAK;
  ...
  if (opt_bus_names != NULL && opt_bus_names->len == 0)
    {
      ...
      const char *steam_appid = _srt_get_steam_app_id (_srt_peek_environ_nonnull ());
