                ...
                GError **error)
{
  ...
  if (path_fd < 0)
    return glnx_fd_throw_errno_prefix (error,
                                       "Failed to open %s to expose in sandbox",
                                       path);
...
choose_implementation (GPtrArray *possible_names,
                       ...
                       GError **error)
{
  gboolean tried_flatpak = FALSE;
  gboolean tried_srls = FALSE;
  const char *detail;
  gsize i;
  ...
  if (tried_srls)
    detail = " (is steam-runtime-launcher-service running?)";
  else if (tried_flatpak)
    detail = " (is Flatpak installed?)";
  else
    detail = "";

  glnx_throw (error,
              "Unable to connect to any of the specified bus names%s", detail);
  return FALSE;
}
...
static const GOptionEntry options[] =
{
  ...
  { "inside-app", '\0',
    ...
    "APPID" },
  { "if-exists", '\0',
    G_OPTION_FLAG_NONE, G_OPTION_ARG_NONE, &opt_if_exists,
    "Suppress error message if the command doesn't exist.",
    NULL },
  { "list", '\0',
  ...
  { "shell-command", 'c',
    G_OPTION_FLAG_NONE, G_OPTION_ARG_FILENAME, &opt_shell_command,
      ...
      char *argv[])
{
...

  {
    ...
    if (reply == NULL)
      {
        if (g_error_matches (local_error, G_DBUS_ERROR, G_DBUS_ERROR_FILE_NOT_FOUND))
          launch_exit_status = LAUNCH_EX_NOT_FOUND;
        else if (g_error_matches (local_error, G_DBUS_ERROR, G_DBUS_ERROR_ACCESS_DENIED)
                 || g_error_matches (local_error, G_DBUS_ERROR, G_DBUS_ERROR_SPAWN_FAILED))
          launch_exit_status = LAUNCH_EX_CANNOT_INVOKE;

        if (opt_if_exists && launch_exit_status == LAUNCH_EX_NOT_FOUND)
          {
            g_debug ("Command not found, ignoring: %s", local_error->message);
