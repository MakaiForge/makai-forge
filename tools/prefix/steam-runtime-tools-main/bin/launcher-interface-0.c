...
{
  const char *requested = getenv (STEAM_COMPAT_LAUNCHER_SERVICE_ENVVAR);
  ...
  if (strchr (requested, ':') != NULL)
    {
      fprintf (stderr,
               "%s: Expected a single entry in $%s\n",
               NAME, STEAM_COMPAT_LAUNCHER_SERVICE_ENVVAR);
      return false;
    }
...
find_launcher_service (void)
{
  ...
  for (i = 0; i < N_ELEMENTS (search_dirs); i++)
    {
      ...
      if (asprintf (&path,
                    "%s/libexec/steam-runtime-tools-0/%s-srt-launcher-service",
                    search_dirs[i],
                    _SRT_MULTIARCH) < 0)
        {
          log_oom ();
  ...
  /* Same, but with an older naming convention */
  for (i = 0; i < N_ELEMENTS (search_dirs); i++)
    {
      autofree char *path = NULL;

      if (asprintf (&path,
                    "%s/bin/steam-runtime-launcher-service",
                    search_dirs[i]) < 0)
        {
          log_oom ();
          return NULL;
...
#ifdef _SRT_MULTIARCH
          if (asprintf (&path,
                        "%s/pressure-vessel/libexec/%s-srt-launcher-service",
                        next,
                        _SRT_MULTIARCH) < 0)
            {
              log_oom ();
...
main (int argc, char **argv)
{
  ...
  if (want_launcher_service (tool_names)
      && (launcher_service = find_launcher_service ()) != NULL)
    {
      ...
      options = _srt_launcher_interface_get_options (NULL);

      for (i = 0; options[i] != NULL; i++)
        {
          if (argz_add (&argz, &argz_len, options[i]) != 0)
            {
              log_oom ();
              goto fallback;
            }
      ...
      unsetenv (STEAM_COMPAT_LAUNCHER_SERVICE_ENVVAR);
