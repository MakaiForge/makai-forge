 ...
 *
 * This wraps g_clear_fd() with the signature required by
                         ...
                         GError **error)
{
  ...
  g_return_val_if_fail (!_srt_all_bits_set (flags,
...

    {
      ...
      if (fd < 0)
        return glnx_fd_throw_errno_prefix (error, "Unable to duplicate fd \"%d\"",
                                           sysroot);
  ...
  while (remaining != NULL)
    {
      ...
      if (fd < 0 && errno == ENOENT && (flags & SRT_RESOLVE_FLAGS_MKDIR_P) != 0)
        {
          if (TEMP_FAILURE_RETRY (mkdirat (g_array_index (fds, int, fds->len - 1),
                                           next, 0700)) != 0)
            return glnx_fd_throw_errno_prefix (error, "Unable to create \"%s/%s\"",
                                               current_path->str, next);
      ...
      if (fd < 0)
        return glnx_fd_throw_errno_prefix (error, "Unable to open \"%s/%s\"",
                                           current_path->str, next);
  ...
  if (flags & SRT_RESOLVE_FLAGS_MUST_BE_EXECUTABLE)
    {
      /* faccessat didn't support AT_EMPTY_PATH until Linux 5.8 and we
       * still support older kernels, so do this instead. */
      g_assert (proc_fd_name != NULL);

      if (access (proc_fd_name, X_OK) != 0)
        return glnx_fd_throw_errno_prefix (error, "\"%s\" is not executable",
                                           current_path->str);
    }
  ...
  if (flags & SRT_RESOLVE_FLAGS_READABLE)
    {
      ...
      if (real_path_out != NULL)
        *real_path_out = g_string_free_and_steal (g_steal_pointer (&current_path));
  ...
  if (_srt_sysroot_is_direct (sysroot))
    {
      ...
      if (flags & SRT_RESOLVE_FLAGS_READABLE)
      ...
      else
        {
          ...
          if (fd < 0)
            return glnx_fd_throw_errno_prefix (error, "Unable to open \"%s\"",
                                               path);
        }
