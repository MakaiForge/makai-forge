...
Please see
[Steam Linux Runtime - guide for game developers][developer mode]
...
**--filesystem** *PATH*[:*MODE*]|*TOKEN*[:*MODE*]

</dt><dd>

Share *PATH* or *TOKEN* from the host system with the container,
similar to the corresponding option in **flatpak-run**(1).

A *MODE* may optionally be specified as a suffix
after a colon,
either `ro` for read-only or `rw` for read/write.
If a suffix is not specified,
the default is `rw`.
`create` is also accepted for compatibility with Flatpak,
but is currently implemented as equivalent to `rw`.

A *PATH* may be specified as either an absolute path,
or a path in the home directory prefixed with `~/`.
Other types of path implemented by Flatpak,
such as `xdg-data/foo`,
are not currently accepted.
If a *PATH* contains colons or backslashes,
they must be escaped as `\:` or `\\` respectively.
(When run from a shell,
this syntax is likely to require quoting or an additional layer of escaping.)

The special token `host` shares all subdirectories of the root directory
with the container.
This is enabled by default when running the Steam Client.

The special tokens `host-etc` and `host-os` are accepted,
but are currently ignored:
when using a runtime,
pressure-vessel always makes `/etc`,
`/usr`,
and `/usr`-like filesystems such as `/bin`
available in the container as `/run/host/etc` and so on
(or as `/run/parent/etc` and so on when running under Flatpak),
and when not using a runtime,
the current `/etc` and `/usr` are used directly.

The special token `host-root` results in the entire host filesystem
being mounted read/write at `/run/host/root`,
as described in systemd's [Container Interface][] document.
This is enabled by default when running the Steam Client.
Note that as with Flatpak, paths within this hierarchy should always be
resolved using **openat2**(2) with the `RESOLVE_IN_ROOT` flag,
or an equivalent interface such as steam-runtime-tools'
`_srt_resolve_in_sysroot()` or libglnx's `glnx_chaseat()`.

The special token `home` is not currently accepted.
`--share-home` has a similar meaning,
but also shares related directories such as `/run/media` and `/srv`.
...
Assume that the *COMMAND* is the Steam Client,
or a similarly tightly-integrated component.

This changes some defaults to be appropriate for the Steam Client:
currently it means that the equivalents of
`--filesystem=host`
and `--filesystem=host-root` are assumed
(unless each filesystem was explicitly specified with a `:ro` suffix),
and `/sys` is mounted read/write instead of read-only.

Other parameters that are appropriate for the Steam Client might be
affected by this option in future.
...
**--graphics-provider** *DIR*|*MANIFEST*, **--graphics-provider** *TUPLE*__=__*DIR*|*MANIFEST*,

</dt><dd>

If using a `--runtime`, use *DIR* or *MANIFEST* to provide graphics drivers.

If the argument is a the absolute path to a regular file,
parse it as a JSON manifest as described in
**steam-runtime-graphics-provider.json**(5)
and use graphics drivers according to the contents of that manifest.
On architectures that are not supported by the graphics provider described
by the manifest,
fall back to using graphics drivers from the real root directory.
...
the root of an operating system installation (a "sysroot").

The default is `$STEAM_COMPAT_GRAPHICS_PROVIDER` if set,
or `/` otherwise.

If prefixed with a multiarch tuple such as **x86_64-linux-gnu**,
use *DIR* or *MANIFEST* for graphics drivers for that architecture only,
overriding any non-architecture-specific choice.
For example,
`--graphics-provider=/ --graphics-provider=i386-linux-gnu=/opt/32`
would take `i386` graphics drivers from the sysroot `/opt/32`,
but `x86_64` graphics drivers from the root directory.
...
Used to locate Mesa GBM backends to be made available in the container
if `--runtime` and `--graphics-provider` are active.
...
`PRESSURE_VESSEL_ARCHITECTURES` (`:`-separated list of multiarch tuples)

</dt><dd>

Experimental option to set the architectures to be supported in the
container.
The first architecture in the list is considered to be the most important
(the primary architecture).
The default is `x86_64-linux-gnu:i386-linux-gnu` if pressure-vessel
was built on an x86 platform (x86\_64 or i386),
or the multiarch tuple for which pressure-vessel was built otherwise.
...
`PRESSURE_VESSEL_EMULATOR` (path to a JSON file)

</dt><dd>

Equivalent to `STEAM_COMPAT_EMULATOR`.
If both are set,
this one takes precedence:
this is intended for developer/debugging use,
rather than being set by Steam.
...
If both this variable and `STEAM_COMPAT_GRAPHICS_PROVIDER` are set,
this variable takes precedence:
it is intended for debugging and development rather than for production use.

</dd>
<dt>

`PRESSURE_VESSEL_GRAPHICS_PROVIDER_x86_64_linux_gnu`, etc.

</dt><dd>

Equivalent to
`--graphics-provider="x86_64-linux-gnu=$PRESSURE_VESSEL_GRAPHICS_PROVIDER"`
and so on.
...
`STEAM_COMPAT_EMULATOR` (path to a JSON file)

</dt><dd>

Experimental option to configure non-transparent CPU emulation,
without needing `binfmt_misc`.
`pressure-vessel-wrap` itself must be directly runnable,
or runnable via transparent emulation.
This variable is intended to be set by Steam during emulation.

This emulator is assumed to be appropriate for the *COMMAND*.

The referenced file must be in **steam-runtime-emulator.json**(5) format.
See [steam-runtime emulator.json(5)][].

The emulator itself should not be included in the *COMMAND*.
...
`STEAM_COMPAT_GRAPHICS_PROVIDER` (absolute path)

</dt><dd>

Equivalent to `--graphics-provider="$STEAM_COMPAT_GRAPHICS_PROVIDER"`,
but with a lower precedence.
This variable is intended to be set by Steam during emulation.
...
`STEAM_COMPAT_RUNTIME_SDL2` (`0`, `1`, `sdl2-compat` or `classic`)
...
If set to `classic`,
and the runtime contains `sdl2-compat` as the default implementation of
SDL 2 but also has a copy of "classic" SDL 2 as a non-default implementation,
do the same for "classic" SDL 2.
...
Set to a search path for Mesa GBM backends
if `--runtime` and `--graphics-provider` are active.
...
[developer mode]: https://gitlab.steamos.cloud/steamrt/steam-runtime-tools/-/blob/main/docs/slr-for-game-developers.md#developer-mode
[steam-runtime emulator.json(5)]: https://gitlab.steamos.cloud/steamrt/steam-runtime-tools/-/blob/main/docs/steam-runtime-emulator.json.5.md
