...
class TestLauncher(BaseTest):
    def setUp(self) -> None:
        ...
        self.clean_up_env_as_bytes = [
            s.encode('utf-8') for s in self.clean_up_env
        ]

        if 'SRT_TEST_UNINSTALLED' in os.environ:
            if 'SRT_TEST_MULTIARCH' in os.environ:
                self.launcher = self.command_prefix + [
                    os.path.join(
                        self.top_builddir,
                        'bin',
                        (
                            os.environ['SRT_TEST_MULTIARCH']
            ...
            self.launch_as_bytes = [s.encode('utf-8') for s in self.launch]
        else:
    ...
    def test_socket_directory(self) -> None:
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                logger.debug('launch-client -c ... should succeed')
                completed = run_subprocess(
                    self.clean_up_env_as_bytes + self.launch_as_bytes + [
                        b'--dbus-address', dbus_address.encode('utf-8'),
                        b'-c',
                        # Note that this is not valid UTF-8, but we want
                        # to make sure we can pass it through anyway.
                        b"printf '\xf6'; echo \"(argv[0]=$0)\"; echo \"$*\"",
                        b'sh',      # $0 for the one-line shell script
                        b'\xf7',    # $1 (again, not UTF-8)
                    ],
                    check=True,
                    stdin=subprocess.DEVNULL,
                    stdout=subprocess.PIPE,
                ...
                logger.debug('Checking execve EACCES reporting')
                completed = run_subprocess(
                    self.clean_up_env + self.launch + [
                        '--socket', socket,
                        '--',
                        # execve() fails with EACCES when we try to
                        # exec a directory
                        '/dev',
                    ],
                    stdin=subprocess.DEVNULL,
                ...
                logger.debug('Checking execve ENOENT reporting')
                completed = run_subprocess(
                    self.clean_up_env + self.launch + [
                        '--socket', socket,
                        '--',
                        os.path.join(temp, 'nonexistent'),
                    ],
                    stdin=subprocess.DEVNULL,
                    stdout=2,
                    stderr=2,
                ...
                logger.debug('Checking execve misc error reporting')
                completed = run_subprocess(
                    self.clean_up_env + self.launch + [
                        '--socket', socket,
                        '--',
                        # execve() will fail with ENOTDIR, which is not an
                        # error that we handle specially
                        '/dev/null/ENOTDIR',
                    ],
                    stdin=subprocess.DEVNULL,
                    stdout=2,
