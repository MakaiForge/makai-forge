...
class TestInsideRuntime(BaseTest):
    ...
    def setUp(self) -> None:
        ...
        self.home_is_unshared = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_IS_HOME_UNSHARED'),
        )
        self.locales_configured = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_LOCALES'),
        )

        self.runtime_is_copy = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_IS_COPY'),
        )
        self.runtime_is_scout = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_IS_SCOUT'),
        )
        self.runtime_is_sdk = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_IS_SDK'),
        )
        self.runtime_is_soldier = bool(
            os.environ.get('TEST_INSIDE_RUNTIME_IS_SOLDIER'),
        )
    ...
    def test_environ(self) -> None:
        ...
        if self.runtime_is_copy:
            ...
            for path in ldlp:
                self.assertTrue(
                    ...
                    '{!r} should end with /aliases'.format(path),
                )
        ...
        if (
            ...
            self.assertDirectory(
                check_dir,
                '{} should be a directory in container'.format(check_dir),
            )
            self.assertDirectory(Path(check_dir, 'aliases'))

            if check_dir not in search:
                check_dir = '/usr/lib/pressure-vessel' + check_dir

            self.assertDirectory(check_dir)
            self.assertDirectory(Path(check_dir, 'aliases'))
            self.assertIn(check_dir, search)
            self.assertIn(check_dir + '/aliases', ldlp)
    ...
    def test_overrides(self) -> None:
        if self.runtime_is_copy:
            target = self.assertSymlink('/overrides')
            self.assertEqual(target, 'usr/lib/pressure-vessel/overrides')

        self.assertDirectory('/overrides')
        self.assertDirectory('/overrides/lib')

        if (
            'HOST_LD_LINUX_SO_REALPATH' in os.environ
            and Path('/usr/lib/i386-linux-gnu').is_dir()
        ):
            self.assertDirectory('/overrides/lib/i386-linux-gnu')

        if (
            'HOST_LD_LINUX_X86_64_SO_REALPATH' in os.environ
            and Path('/usr/lib/x86_64-linux-gnu').is_dir()
        ):
            self.assertDirectory('/overrides/lib/x86_64-linux-gnu')

        if self.runtime_is_copy:
            # /etc/ld.so.* are symlinks to the mutable version
            target = self.assertSymlink('/etc/ld.so.cache')
            self.assertEqual(target, '/var/pressure-vessel/ldso/ld.so.cache')
            target = self.assertSymlink('/etc/ld.so.conf')
            self.assertEqual(target, '/var/pressure-vessel/ldso/ld.so.conf')

            # Exherbo compatibility symlinks also exist
            target = self.assertSymlink('/etc/ld-i686-pc-linux-gnu.cache')
            self.assertEqual(target, '/var/pressure-vessel/ldso/ld.so.cache')
            target = self.assertSymlink('/etc/ld-x86_64-pc-linux-gnu.cache')
            self.assertEqual(target, '/var/pressure-vessel/ldso/ld.so.cache')
            # Clear Linux compatibility symlinks, too
            target = self.assertSymlink('/var/cache/ldconfig/ld.so.cache')
            self.assertEqual(target, '/var/pressure-vessel/ldso/ld.so.cache')
            ...
            if (
                ...
                for really in (
                    ...
                    self.assertEqual(
                        ...
                        (really_stat.st_dev, really_stat.st_ino),
                        '{} and {} should be the same device/inode'.format(
                            expected, really,
                        ),
                    )
    ...
    def test_srsi(self) -> None:
        ...
        with self.catch(
            ...
            self.assertNotIn('on-network-filesystem', issues)
            self.assertNotIn('on-non-unix-filesystem', issues)
            self.assertNotIn('on-unknown-filesystem', issues)
            self.assertNotIn('unexpected-location', issues)
            self.assertNotIn('unexpected-version', issues)
            self.assertNotIn('unknown', issues)
            ...
            if self.locales_configured:
                for locale, host_details in host_parsed.get(
                    ...
                    if 'resulting-name' in host_details:
                        if (
                            locale == '<default>'
                            and host_details['resulting-name'] == 'C'
                            and host_details != details
                        ):
                            # Newer versions of Python force LC_CTYPE
                            # to be C.UTF-8 as a result of PEP-0538,
                            # and this is a Python script, so we can't
                            # assert that the default locale is still C:
                            # it could be something more useful now
                            self.assertIn('resulting-name', details)
                            self.assertEqual(details.get('charset'), 'UTF-8')
                            self.assertEqual(details.get('error'), None)
                            self.assertEqual(details.get('is_utf8'), True)
                        else:
        ...
        for multiarch in parsed['architectures']:
            ...
            for soname, details in arch_info['library-details'].items():
                with self.catch(
                    ...
                    if (
                        ...
                        and self.runtime_is_scout
                        and self.runtime_is_sdk
                        and os.path.exists('/lib32/libcrypt.so.1')
                        and multiarch == 'i386-linux-gnu'
                    ):
                        # Even though we prefer to use libxcrypt's libcrypt
                        # implementation, gcc-*-multilib pulls in
                        # libc6{,-dev}-i386 which includes
                        # /lib32/libcrypt.so.1 which has a smaller ABI.
                        expect_library_issues |= set(details.get('issues', []))
                        continue
            ...
            if host_info:
                overrides_libdir = '{}/lib/{}'.format(
                    overrides, multiarch
                )

                self.assertGraphicsModulesOverrides(
                    modules=host_info.get('dri_drivers', ()),
                    module_type='dri',
                    multiarch=multiarch,
                ...
                self.assertGraphicsModulesOverrides(
                    modules=host_info.get('gbm_backends', ()),
                    module_type='gbm',
                    multiarch=multiarch,
                    overrides_libdir=overrides_libdir,
                )
                    ...
                    with self.subTest(stack=stack):
                        ...
                        for key in (
                            ...
                            with self.subTest(key=key):
                                host_value = host_details.get(key)

                                # An unknown library vendor or a missing
                                # library is represented as "unknown",
                                # whereas an unknown or non-functioning
                                # renderer is represented as null (None).
                                # Normalize to None.
                                if (
                                    key == 'library-vendor'
                                    and host_value == 'unknown'
                                ):
                                    host_value = None

                                # If the host library can't be loaded or
                                # identified, we can't assert that the same
                                # will be true in the container:
                                # SLR has a fallback copy of Mesa,
                                # with at least software rendering available.
            ...
            with self.catch(
                ...
                self.assertEqual(
                    ...
                    set(arch_info.get('library-issues-summary', ())),
                )
