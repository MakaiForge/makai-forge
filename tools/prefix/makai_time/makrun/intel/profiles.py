...
Estrutura do perfil:
{
    ...
    "runtime": "makai-runtime",      # runtime recomendado
...
def get_profile(game_id: str) -> dict | None:
    ...
    for profile in GAME_PROFILES:
        p_slug = profile.get("name", "unknown").lower().replace(" ", "-").replace(":", "").replace("'", "")
        if p_slug == slug:
            return profile
    return None
...
def save_registry_to_disk(base_path: str):
    ...
    _profiles_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "profiles", "data")
