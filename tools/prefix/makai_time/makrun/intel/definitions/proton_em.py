...
DEFINITION = {
    ...
    "patches": [
        'winewayland_hdr', 'winewayland_color_management',
        'winewayland_mouse_pointer', 'fsr4', 'media_fixes',
        'protonfixes', 'media_conversion',
    ],
    "dll_overrides": {
        "beclient.dll": "b,n",
    ...
    "notes": "Wayland-first Proton with HDR support. FSR4 pioneer. winewayland.drv with color management.",
    "container_overrides": {"ntsync": True, "nvidia_libs_bundled": True},
}


def get_container_config() -> dict:
    """Container config for Proton-EM (Endless Mirage).

    HDR via winewayland.drv patches. NTSYNC via WINENTSYNC.
    Uses wine64 (não wine-preloader). NVIDIA libs bundled.
    """
    return {
        "audio": {
